...
public class LevelManager {
	...
	public Optional<String> get(String row) throws InterruptedException, NoSuchElementException {
		...
		if (index != -1) {
			...
			DataBlockLoader dataBlockLoader = new DataBlockLoader(dataBlock, config.getPerBlockBloomFilterBits(),
					config.getHasher());
	...
	public Map<String, Timed<String>> getColumnWithQualifier(Qualifier q) throws IOException, InterruptedException {
		...
		for (DataBlock db : dataBlocks) {
			DataBlockLoader dbLoader = new DataBlockLoader(db, config.getPerBlockBloomFilterBits(), config.getHasher()); 
	...
	private TempDataBlock[] getTempDataBlocks() {
		String[] filenames = getColumnDir().list((dir, name) -> TempDataBlock.isTempDataBlock(name, config));
		if (filenames == null) {
	...
	/**
	 * Renames files. For example: DataBlock: 1_0_Data.db, 1_1_Data.db,
	 * 1_3_Data.db, 1_5_Data.db
	 * <p>
	 * TempDataBlock: 1_2_Data.db.tmp_0, 1_2_Data.db.tmp_1, 1_2_Data.db.tmp_2,
	 * 1_4_Data.db.tmp0
	 * <p>
	 * The method will merge them and collect them like: 1_0_Data.db, ->
	 * 1_0_Data.db 1_1_Data.db, -> 1_1_Data.db 1_2_Data.db.tmp_0, -> 1_2_Data.db
	 * 1_2_Data.db.tmp_1, -> 1_3_Data.db 1_2_Data.db.tmp_2, -> 1_4_Data.db
	 * 1_3_Data.db, -> 1_5_Data.db 1_4_Data.db.tmp_0, -> 1_6_Data.db 1_5_Data.db
	 * -> 1_7_Data.db
	 * <p>
	...
	private Modifications collect() throws IOException {
		...
		// requires the compact() method to remove original data files that are
		// compacted
		ArrayList<File> files = mergeTempAndDataBlocks();
		for (int i = files.size() - 1; i >= 0; i--) { // must from higher index
														// to lower
	...
	private Modifications load(DataBlock b) throws IOException {
		DataBlockLoader loader = new DataBlockLoader(b, config.getPerBlockBloomFilterBits(), config.getHasher());
	...
	private void dumpTemp(Modifications m, int index, int origIndex) throws IOException {
		...
		Filter f = m.calculateFilter(new BloomFilter(config.getPerBlockBloomFilterBits(), config.getHasher()));
	...
	/**
	 * Compacts a block into this level. In case that the total numLongs exceeds
	 * the limit, return the block needs to push down. Note that config should
	 * be used.
	 *
	 * @param block
	 *            the block to compact in this level
