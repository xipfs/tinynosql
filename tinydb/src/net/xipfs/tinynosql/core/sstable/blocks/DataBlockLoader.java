...
public class DataBlockLoader extends AbstractSSTableBlock {
	...
	public DataBlockLoader(DataBlock block, int bloomFilterBits, StringHasher hasher, boolean compressible) {
		dataBlock = block;
	...
	public Modification get(String row) throws NoSuchElementException {
		...
		try {
			c = compressible ? dataBlock.getCompressibleReadableComponentFile() : dataBlock.getReadableComponentFile();
			...
			throw new NoSuchElementException("could not find element due to CompressionException " + ce.getMessage());
		} finally {
