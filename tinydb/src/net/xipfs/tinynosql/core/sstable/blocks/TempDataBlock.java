...
public class TempDataBlock extends AbstractBlock implements Comparable<TempDataBlock> {
	...
	public TempDataBlock(Descriptor desc, String column, int level, int index, int originIndex, SSTableConfig config) {
		super(config);
		this.desc = desc;
		this.level = level;
		this.index = index;
	...
	public static Optional<TempDataBlock> fromFileName(Descriptor desc, String column, String filename,
	...
	public File getFile() throws IOException {
		...
		// <level>_<originIndex>_Data.db.tmp_<index>
		String filename = String.format("%d_%d_Data%s_%d", level, originIndex, config.getTempBlockFilenameSuffix(),
				index);
	...
	public ComponentFile getCompressibleWritableComponentFile() throws IOException, CompressorException {
		requireFileWritable();
		return new ComponentFile(getFile(), "w", config.getFileBufferSize(), config.getCompressorProvider(),
